...
const JournalSubmissionButton = function({ id }) {
    ...
    const submitToJournal = function() {
        ...
        setRequestId(dispatch(postJournalSubmissions(journal.id, submission)))
    }
            ...
            <>
                ...
                <Modal className="journal-submission-modal" isVisible={modalIsVisible} setIsVisible={setModalIsVisible}>
                    ...
                    <div className="selected">
                        { journal && <JournalTag id={journal.id} onRemove={(e) => setJournal(null)} /> }
                    </div>
                    { ! journal && <JournalSelectionInput 
                        journal={journal}
                        setJournal={setJournal}
                    /> }
                    <Button type="secondary-warn" onClick={(e) => setModalIsVisible(false)}>Cancel</Button> 
                    <Button type="primary-highlight" disabled={ journal ? false : true } onClick={(e) => submitToJournal()}>Submit to Journal</Button> 
                </Modal>
            </>
        )
    } else {
